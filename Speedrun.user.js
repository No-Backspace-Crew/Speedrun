  ...
  <details-dialog class="Box height-fit overflow-auto Box-overlay--wide anim-fade-in fast">
    ...
    function isEnabledWikiPath() {
        ...
        if (regex) {
            ...
            return GM_getValue(SR_ENABLED_PATHS, {})[path] || path === '/no-backspace-crew/speedrun' || path === '/no-backspace-crew/warp';
        }
    ...
    function updateTabs() {
        $('.srRunBtn').each(async function (item) {
            ...
            if(variables.creds) {
                btn.attr('aria-label', `${variables.srServiceName}${variables.service != getService()? " 📌":""}: ${variables.region}`);
