  ...
  <details-dialog class="Box height-fit overflow-auto Box-overlay--wide anim-fade-in fast">
    ...
    <div class="Box-footer text-right">
      <button id="srModal-ok" type="button" class="btn btn-primary" autofocus data-close-dialog>Ok</button>
            ...
            $('#srFederate,#srFederateService').on('click.sr', async (event) => {
            ...
            $("#srGetCreds").on('click', async (event) => {
    ...
    function isAlwaysOnWikiPath(path) {
        if(!path) {
          let regex = isWiki();
          if (regex) {
            [,path] = regex;
    ...
    function isEnabledWikiPath() {
        ...
        if (regex) {
            ...
            return GM_getValue(SR_ENABLED_PATHS, {})[path] || isAlwaysOnWikiPath(path);
        }
        return false;
    }
    ...
    async function updatePage(reason) {
        ...
        try {
            ...
            isAlwaysOnWikiPath(path) ? $('#srToggleTitle').hide() : $('#srToggleTitle').show();
            ...
            newServices.length > 0 ? serviceDropdown.show() && serviceDropdown.next().show() : serviceDropdown.hide() && serviceDropdown.next().hide();
            ...
            for(const block of $("p > code, li > code, td > code").not('code + span.copyCursor')) {
